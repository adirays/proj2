class ConfigService {
  ...
  double getCalorieAdjustment(String key) {
    return _settings["calorie_adjustments"][key].toDouble();
  }

  double getMinCalories(String gender) {
    return _settings["min_calories"][gender].toDouble();
  }

  Map<String, double> getMacroPercentages(String condition) {
    Map<String, dynamic> percentages = _settings["macro_percentages"][condition] ?? 
                                      _settings["macro_percentages"]["general"];
    return percentages.map((key, value) => MapEntry(key, value.toDouble()));
  }

  Map<String, String> getMicronutrientGuidelines(String condition) {
    Map<String, dynamic> guidelines = _settings["micronutrient_guidelines"][condition] ?? 
                                     _settings["micronutrient_guidelines"]["general"];
    return guidelines.map((key, value) => MapEntry(key, value.toString()));
  }
}
